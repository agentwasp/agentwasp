...
- **Supermemory-first persistent memory**: this branch defaults to `MEMORY_BACKEND=supermemory`, stores conversational turns through Supermemory, and injects one compact profile/search block instead of Agent Wasp's local conversational recall. The local memory/graph dashboard surfaces remain available for compatibility and migration visibility.
